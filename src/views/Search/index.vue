  ...
  export default {
    ...
    methods: {
      ...
      removeCategoryName() {
        ...
        //骚操作:路由自己跳自己,有params参数就携带，没有就光跳转到search
      ...
      removeKeyword() {
        ...
        //通知兄弟组件清除search框中的关键字
        this.$bus.$emit("clear");
        //为什么这里没有调用发请求函数？
      },
      //子组件给父组件传递数据的自定义事件的回调
      getTradeMark(tmId, tmName) {
        console.log("父组件拿到了点击的品牌数据", tmId, tmName);
        //整理品牌相关的搜索条件
      ...
      getAttrAndAttrValue(attrId, attrName, attrValue) {
        ...
        console.log('拿到了搜索的属性',newProps)
        if (this.searchParams.props.indexOf(newProps) == -1) { // 当期那里米娜没有
    ...
    changeOrder(flag) {
      // flag:用户每一次点击li标签在时候，用于区分椒综合(1)还是价格(2)
